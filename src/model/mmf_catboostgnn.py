...
class ProteinEmbeddingLoader(nn.Module):
    def __init__(self, base_path: Path):
        super().__init__()
        self.base_path = base_path
        self.protein_ids = {}
        
    def forward(self, protein_idx_list: List[str]) -> torch.Tensor:
        protein_embeddings = []
        
        for protein_idx in protein_idx_list:
            if protein_idx in self.protein_ids:
                protein_embeddings.append(self.protein_ids[protein_idx])
            else:
                protein_path = self.base_path / f"{protein_idx}.pt"
                protein_embedding = torch.load(protein_path)
                protein_embeddings.append(protein_embedding)
                self.protein_ids[protein_idx] = protein_embedding
...
class MMFCatboostGNN(ModelWrapper):
    def __init__(self, cfg: DictConfig, task: str):
        ...
        self._load_encoders()
        self._load_mix_layer()
    
    # TODO: Implement this function
    def _load_encoders(self):
        if self.cfg.model.molecule_encoder.name == 'fingerprint':
            for fingerprint_type in self.cfg.model.molecule_encoder.type:
                print(fingerprint_type)
        
        elif self.cfg.model.molecule_encoder == 'fingerprint-precomputed':
            pass
        
        else:
            raise ValueError(f"Unsupported molecule encoder: {self.cfg.model.molecule_encoder}")
        
        
        if self.cfg.model.protein_encoder.name == 'esm-precomputed':
            self.protein_embedding_loader = ProteinEmbeddingLoader(base_path=Path(self.cfg.model.protein_encoder.base_path))
        
        elif self.cfg.model.protein_encoder == 'esm':
            pass
        
        else:
            raise ValueError(f"Unsupported protein encoder: {self.cfg.model.protein_encoder}")
                

    # TODO: Implement this function
    def _load_mix_layer(self):
        # self.mix_layer = 
        
        pass
    
    # TODO: Implement this function
    def mix_multi_modal_embeddings(self, x_molecule: torch.Tensor, x_protein: torch.Tensor) -> torch.Tensor:
        x_molecule = self.molecule_encoder(x_molecule)
        x_protein = self.protein_encoder(x_protein)

        x = self.mix_layer(x_molecule, x_protein)
        
        return x
    
    
    def train(self):
        for seed in range(self.cfg.job.max_seed):
            # train, test = self.group.get_train_valid_split(benchmark = self.task, split_type = 'default', seed = seed)
            # train_loader = DataLoader(train, batch_size=self.cfg.model.batch_size, shuffle=True)
            # test_loader = DataLoader(test, batch_size=self.cfg.model.batch_size, shuffle=False)
            
            for epoch in range(self.cfg.model.epochs):
                pass
                # self.model.save(self.model, seed)
